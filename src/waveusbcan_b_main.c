...
/** Manual endpoint override parameters. Negative values mean auto/profile
 * selection. */
...
MODULE_PARM_DESC(ch0_cmd_ep,
                 "Channel 0 command endpoint number, without 0x80 IN bit");
MODULE_PARM_DESC(ch0_msg_ep,
                 "Channel 0 CAN message endpoint number, without 0x80 IN bit");
MODULE_PARM_DESC(ch1_cmd_ep,
                 "Channel 1 command endpoint number, without 0x80 IN bit");
MODULE_PARM_DESC(ch1_msg_ep,
                 "Channel 1 CAN message endpoint number, without 0x80 IN bit");
...
MODULE_PARM_DESC(poll_interval_ms,
                 "RX polling interval in milliseconds (default: 20)");
...
MODULE_PARM_DESC(usb_retries,
                 "USB bulk retry count for transient -EAGAIN/-ETIMEDOUT/-EPIPE "
                 "startup errors (default: 3)");

/** Verbose channel-open diagnostics. */
static bool debug_open = true;
module_param(debug_open, bool, 0644);
MODULE_PARM_DESC(debug_open,
                 "Log each netdev open/startup step to dmesg (default: true)");
...
MODULE_PARM_DESC(usb_timeout_ms,
                 "USB bulk transfer timeout in milliseconds (default: 1000)");
...
MODULE_PARM_DESC(
    autopm_on_open,
    "Try to hold a USB runtime-PM reference while CAN netdev is open (default: "
    "false; usb_disable_autosuspend() is already used while bound)");
...
MODULE_PARM_DESC(
    autopm_strict,
    "Fail CAN open if usb_autopm_get_interface() fails (default: false)");
...
MODULE_PARM_DESC(
    command_ep_autodetect,
    "On open, retry COMMAND_INIT on other bidirectional bulk endpoint numbers "
    "if the selected command endpoint fails (default: true)");

static bool device_reset_on_disconnect = false;
module_param(device_reset_on_disconnect, bool, 0644);
MODULE_PARM_DESC(device_reset_on_disconnect,
                 "Send vendor command 0x07 during driver disconnect/unload "
                 "(default: false)");

static int device_reset_settle_ms = 250;
module_param(device_reset_settle_ms, int, 0644);
MODULE_PARM_DESC(device_reset_settle_ms,
                 "Delay after optional vendor device reset, clamped to 0..5000 "
                 "ms (default: 250)");

/**
 * Forward declaration for the USB parent object shared by both CAN channels.
 */
struct wuc_device;
...
static int wuc_usb_bulk_write(struct wuc_priv *priv, u8 ep, u8 *buf, int len) {
  ...
  for (attempt = 1; attempt <= max_attempts; attempt++) {
    actual = 0;
    ret = usb_bulk_msg(wdev->udev, pipe, buf, len, &actual, timeout);
    if (!ret && actual == len)
      return 0;
 ...
 */
static int wuc_send_simple_command_on_ep(struct wuc_priv *priv, u8 cmd_ep,
                                         u32 command) {
  u8 *buf;
  int ret;
...
static int wuc_send_vendor_device_reset_on_disconnect(struct wuc_device *wdev) {
  u8 *buf;
  unsigned int pipe;
  int settle = device_reset_settle_ms;
  int actual = 0;
  int ret;

  if (!device_reset_on_disconnect)
    return 0;

  if (!wuc_bidirectional_ep_present(wdev, wdev->cmd_ep[0]))
    return -ENODEV;
  ...
  wuc_build_simple_command(buf, WUC_CMD_DEVICE_RESET);

  pipe = usb_sndbulkpipe(wdev->udev, wdev->cmd_ep[0]);

  mutex_lock(&wdev->usb_lock);
  ret = usb_bulk_msg(wdev->udev, pipe, buf, WUC_USB_PACKET_SIZE, &actual,
                     usb_timeout_ms < 100 ? 100 : usb_timeout_ms);
  mutex_unlock(&wdev->usb_lock);
 ...
 * Return: 0 on success, otherwise negative errno.
 */
static int wuc_get_msg_status(struct wuc_priv *priv, u32 *rx_pending,
                              u16 *tx_pending) {
...
static int wuc_send_init_with_endpoint_fallback(struct wuc_priv *priv) {
  ...
  netdev_warn(priv->netdev,
              "COMMAND_INIT on selected cmd_ep=%u failed: %d; trying fallback "
              "command endpoints\n",
              original, first_ret);
  ...
  for (i = 0; i < ARRAY_SIZE(candidates); i++) {
    ...
    if (!ret) {
      ...
      netdev_info(priv->netdev,
                  "using fallback cmd_ep=%u after successful COMMAND_INIT\n",
                  ep);
      return 0;
    }
...
static int wuc_set_bittiming(struct net_device *netdev) {
  ...
  if (!timing) {
    netdev_err(
        netdev,
        "unsupported bitrate %u; use one of the documented fixed rates\n",
        bitrate);
    return -EINVAL;
  }
 ...
 */
static netdev_tx_t wuc_start_xmit(struct sk_buff *skb,
                                  struct net_device *netdev) {
  struct wuc_priv *priv = netdev_priv(netdev);
  struct wuc_device *wdev = priv->parent;
...
static int wuc_open(struct net_device *netdev) {
  ...
  if (debug_open)
    netdev_info(netdev,
                "open requested: channel=%u cmd_ep=%u msg_ep=%u bitrate=%u "
                "timing_valid=%d\n",
  ...
  if (!priv->timing_valid) {
    ...
    netdev_info(
        netdev,
        "no bitrate configured; defaulting to 125000 for R-Net bench use\n");
  }
 ...
 */
static int wuc_apply_endpoint_profile(struct usb_interface *intf,
  ...
  if (!cmd0 || !msg0 || !cmd1 || !msg1) {
    dev_err(&intf->dev, "manual endpoint profile requires "
                        "ch0_cmd_ep/ch0_msg_ep/ch1_cmd_ep/ch1_msg_ep\n");
    return -EINVAL;
  }
  ...
  if (!wuc_ep_present(wdev->present_bulk_out, cmd0) ||
    ...
    dev_err(&intf->dev,
            "selected endpoints unavailable: ch0 cmd=%u msg=%u, ch1 cmd=%u "
            "msg=%u\n",
            cmd0, msg0, cmd1, msg1);
    return -ENODEV;
  }

  dev_info(&intf->dev, "bulk endpoint map: IN=0x%lx OUT=0x%lx\n",
 ...
 */
static void wuc_scan_endpoints(struct usb_interface *intf,
                               struct wuc_device *wdev) {
...
static int wuc_register_channel(struct wuc_device *wdev, int channel) {
  ...
  /* Linux CAN core accepts either fixed bitrate_const or bittiming_const, not
   * both. This adapter uses the firmware-supported fixed bitrate table. */
  ...
  netdev_info(netdev,
              "registered waveUSBCAN_b channel %d cmd_ep=%u msg_ep=%u\n",
...
static int wuc_probe(struct usb_interface *intf,
...
static void wuc_disconnect(struct usb_interface *intf) {
  ...
  // if (device_reset_on_disconnect) {
  // 	int ret = wuc_send_vendor_device_reset_on_disconnect(wdev);

  // 	if (ret)
  // 		dev_warn(&intf->dev,
  // 			 "vendor device reset command 0x%02x during disconnect
  // failed: %d\n", 			 WUC_CMD_DEVICE_RESET, ret); 	else 		dev_info(&intf->dev,
  // 			 "vendor device reset command 0x%02x during disconnect
  // ok\n", 			 WUC_CMD_DEVICE_RESET);
  // }
